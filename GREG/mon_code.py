...
for q in range(len(questions_melange)):
    ...
    while True:
        ...
        if reponse.isdigit() and reponse <= str(r) and reponse > str(0):
            break
        else:
    ...
    if questions_melange[q][1][reponse-1][1] == True:
        reponse_player.append([1])             #Le joueur à juste, on ajoute 1 à la liste reponse player

    if questions_melange[q][1][reponse-1][1] == False:
        reponse_player.append([0])             #Le joueur a faux, on ajoute 0 à la liste reponse player
